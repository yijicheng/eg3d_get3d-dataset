...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while not SUCC and retry < 5:
            try:
                ...
                SUCC = False
                retry += 1
                idx = np.random.randint(0, len(self._raw_idx))
        ...
        return np.ascontiguousarray(img), self.get_label(idx) # condinfo, np.ascontiguousarray(mask)
    ...
    def _load_raw_labels(self):
        # return None
        if os.path.exists(os.path.join(self.camera_root, 'labels_shapenet_chair.npz')):
            labels = np.load(os.path.join(self.camera_root, 'labels_shapenet_chair.npz'))["labels"]
        else:
            labels = []
